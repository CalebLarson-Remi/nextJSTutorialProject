...
export async function testDatabaseConnection() {
  try {
    ...
    // console.log("Number of customers:", customerCount[0].count);

    // Test invoices table
    const invoiceCount = await sql`SELECT COUNT(*) FROM invoices`;
    // console.log("Number of invoices:", invoiceCount[0].count);

    // Test a sample invoice with customer data
    const sampleInvoice = await sql`
      SELECT 
        invoices.id,
        invoices.amount,
        invoices.date,
